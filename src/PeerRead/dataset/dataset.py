...
def make_dataset_fn_from_file(input_files_or_glob, seq_length,
    ...
    def input_fn(params):
        ...
        # Manteniamo SOLO le feature BERT che servono al pretraining;
        # questo evita che qualunque tf.string entri nel grafo/optimizer.
        dataset = dataset.map(
            lambda features, labels: (
                {
                    "input_word_ids":    tf.cast(features["token_ids"], tf.int32),
                    "input_mask":        tf.cast(features["token_mask"], tf.int32),
                    "input_type_ids":    tf.cast(
                        features["segment_ids"] if "segment_ids" in features
                        else tf.zeros_like(features["token_ids"]), tf.int32),
                    "masked_lm_positions": tf.cast(features["masked_lm_positions"], tf.int32),
                    "masked_lm_ids":       tf.cast(features["masked_lm_ids"], tf.int32),
                    "masked_lm_weights":   tf.cast(features["masked_lm_weights"], tf.float32),
                    # Se dataset_processing fornisce anche next_sentence_labels, passa pure:
                    # "next_sentence_labels": tf.cast(features["next_sentence_labels"], tf.int32),
                },
                labels
            ),
            num_parallel_calls=tf.data.AUTOTUNE
