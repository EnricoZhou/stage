# run_nn.py
# ==============================================================================
# Runner per NN (baseline) per Tabella 3
# ==============================================================================
...
flags.DEFINE_string("prediction_file", "../out/PeerRead/nn/predictions_nn.tsv", "file TSV output predizioni")
...
flags.DEFINE_integer("num_epochs", 200, "numero di epoche")
...
# ---------------- MODEL ---------------- #
def _get_nn_model(vocab_size, hidden_dim, binary_outcome=True):
    ...
    model = tf.keras.Model(inputs=inputs, outputs=[g, q0, q1], name="nn_model")

    model.compile(
        optimizer=tf.keras.optimizers.Adam(learning_rate=FLAGS.learning_rate),
        loss={"g": "binary_crossentropy", "q0": "mse", "q1": "mse"},
...
def parse_bow_example(example_proto):
    feature_description = {
        ...
        # opzionali (presenti solo se hai usato make_nn_tfrecord con y0,y1)
...
def make_dataset(is_training=True, return_extras=False):
    ...
    def add_labels(example):
        t = tf.cast(example["treatment"], tf.float32)
        y = tf.cast(example["outcome"], tf.float32)
        bow = example["bow"]
...
def main(_):
    ...
    model = _get_nn_model(vocab_size=FLAGS.vocab_size, hidden_dim=FLAGS.hidden_dim, binary_outcome=True)
    ...
    eval_ds = eval_ds_full.map(lambda f, l: (f, {k: l[k] for k in ["g", "q0", "q1"]}))
    outputs = model.predict(eval_ds)
    g_pred, q0_pred, q1_pred = outputs
    ...
    # se i valori y0/y1 sono validi (non default -1), aggiungili
