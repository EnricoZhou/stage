...
class LoggingMetricHook(tf.keras.callbacks.Callback):
    """
    Keras-compatible replacement for the old tf.estimator.LoggingTensorHook.
    Logs metrics every `every_n_iter` batches.
    """

    def __init__(self, metric_logger=None, every_n_iter=100):
        ...
        if self._iter_count % self.every_n_iter == 0:
            ...
            for tag, value in logs.items():
                if self.metric_logger is not None:
                    self.metric_logger.log_metric(tag, float(value), global_step=step)
                else:
                    print(f"Step {step}: {tag} = {value:.4f}")

    def on_epoch_end(self, epoch, logs=None):
        """Optional: log metrics also at the end of each epoch."""
        logs = logs or {}
        for tag, value in logs.items():
            if self.metric_logger is not None:
                self.metric_logger.log_metric(tag, float(value), global_step=epoch)
            else:
                print(f"Epoch {epoch+1}: {tag} = {value:.4f}")
