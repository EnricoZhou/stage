...
class ExamplesPerSecondHook(tf.keras.callbacks.Callback):
    """
    Replacement for tf.estimator.SessionRunHook.
    Logs how many examples per second are processed during training.
    """

    def __init__(self, batch_size, every_n_steps=100):
    ...
    def on_batch_end(self, batch, logs=None):
        ...
        if self._global_step % self.every_n_steps == 0:
            ...
            examples_per_sec = (self._global_step * self.batch_size) / elapsed
            print(f"[ExamplesPerSecondHook] step {self._global_step}: {examples_per_sec:.2f} examples/sec")
