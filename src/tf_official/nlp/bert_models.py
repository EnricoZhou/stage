...
class BertPretrainLayer(tf.keras.layers.Layer):
    ...
    def build(self, unused_input_shapes):
        ...
        self.lm_layer_norm = tf.keras.layers.LayerNormalization(
            axis=-1, epsilon=1e-12, name='predictions_transform_LayerNorm')
    ...
    def __call__(self,
        ...
        return super(BertPretrainLayer, self).__call__(inputs, training=training, **kwargs)
...
class BertPretrainLossAndMetricLayer(tf.keras.layers.Layer):
    ...
    def __init__(self, bert_config, **kwargs):
        # AGGIUNTO
        self.masked_lm_accuracy_tracker = tf.keras.metrics.Mean(name="masked_lm_accuracy")
        self.lm_example_loss_tracker = tf.keras.metrics.Mean(name="lm_example_loss")
        self.lm_loss_tracker = tf.keras.metrics.Mean(name="lm_loss")
        self.lm_accuracy_tracker = tf.keras.metrics.Mean(name="lm_accuracy")
        self.next_sentence_accuracy_tracker = tf.keras.metrics.Mean(name="next_sentence_accuracy")
        self.next_sentence_loss_tracker = tf.keras.metrics.Mean(name="next_sentence_loss")
    ...
    def _add_metrics(self, lm_output, lm_labels, lm_label_weights,
        ...
        """Adds metrics using Keras 3 style metric trackers."""
        ...
        self.masked_lm_accuracy_tracker.update_state(masked_lm_accuracy)

        # LM example loss
        lm_example_loss = tf.reshape(lm_per_example_loss, [-1])
        lm_example_loss = tf.reduce_mean(lm_example_loss * lm_label_weights)
        self.lm_example_loss_tracker.update_state(lm_example_loss)

        # Next sentence accuracy
        next_sentence_accuracy = tf.keras.metrics.sparse_categorical_accuracy(
            sentence_labels, sentence_output
        )
        next_sentence_accuracy = tf.reduce_mean(next_sentence_accuracy)
        self.next_sentence_accuracy_tracker.update_state(next_sentence_accuracy)

        # Next sentence loss
        next_sentence_mean_loss = tf.reduce_mean(sentence_per_example_loss)
        self.next_sentence_loss_tracker.update_state(next_sentence_mean_loss)

    @property
    def metrics(self):
        return [
            self.masked_lm_accuracy_tracker,
            self.lm_example_loss_tracker,
            self.lm_loss_tracker,
            self.lm_accuracy_tracker,
            self.next_sentence_accuracy_tracker,
            self.next_sentence_loss_tracker,
    ]
